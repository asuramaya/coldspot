...
# (docs/RELEASE-SIGNING.md, ~/code/REPOS/RELEASE.md). Generates a throwaway
# (non-hardware) ed25519 key to prove the ssh-keygen -Y sign/verify roundtrip
# itself is wired correctly — verification doesn't care what backed the real
# signing key, only that a valid signature exists, so this is a faithful test
# of the mechanism the real FIDO2 key will use. Skips (not fails) if
...
COLDSPOT_UPDATE_SOURCED=1 source "$HERE/src/bin/coldspot-update"
...
# principal = WHO (the repo's identity, "coldspot"); namespace = WHAT-FOR
# ("coldspot-release") — deliberately distinct, per RELEASE.md's identity-
# vs-role split (conflating the two was the bug alfred's review caught).
PRINCIPAL="coldspot"
NS="coldspot-release"
keyfile="$td/id_test"
...
verify_signature "$data" "$sig" "$signers" "not-coldspot" "$NS" \
